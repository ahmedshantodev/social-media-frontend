...
const ProfileUploadModal = ({ show, setShow, user, profileInfo }) => {
  const descriptionRef = useRef();
  const [text, setText] = useState("");
  const [image, setImage] = useState("");
  const [isSeeMore, setIsSeeMore] = useState(false);

  const allProfilePicture = profileInfo?.images?.resources?.filter((item) => {
          ...
          {allProfilePicture?.length > 1 && (
            <div className="mt-4 w-[550px]">
              ...
              <div className="w-full max-h-[300px] overflow-y-auto flex items-start flex-wrap gap-[6px] mt-2 overflow-hidden">
                {allProfilePicture
                  ?.slice(0, isSeeMore ? allProfilePicture?.length : 4)
                  .map((item, index) => (
                    <div
