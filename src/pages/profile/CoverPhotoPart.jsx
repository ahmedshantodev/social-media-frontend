...
const CoverPhotoPart = ({ user, data, visitor, coverPhoto, setCoverPhoto }) => {
  ...
  const allCoverPhoto = data?.images?.resources?.filter((item) => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (response?.data?.message) {
        localStorage.setItem("user", JSON.stringify({ ...user, coverPhoto: response?.data?.url[0] }));
    ...
    <>
      ...
      {coverPhoto && (
        <div className="absolute bottom-0 left-0 z-20 w-full bg-white/40 py-3 px-5 flex items-center justify-end gap-3">
          {loading || isLoading ? (
            <SecondaryButton className={`!px-6 h-[45px] cursor-not-allowed`}>
              cancel
            </SecondaryButton>
          ) : (
            <SecondaryButton
